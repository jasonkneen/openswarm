...
# Delta tracking state — tracks last-seen 9Router totals to compute increments
...
# Threshold to distinguish float jitter from a real 9Router restart
...
def _compute_delta(current: float, last: float | None, threshold: float = _RESTART_THRESHOLD) -> tuple[float, float]:
    ...
    if current < last - threshold:
        # Significant drop — 9Router restarted, treat current as new delta
        return current, current
    if current < last:
        # Tiny drop — float jitter, ignore
        return 0.0, last
    return current - last, current


async def _heartbeat_loop():
    """Send a heartbeat event every 60 seconds for usage-time tracking and cost deltas."""
    ...
    while True:
        ...
        try:
            ...
            # Dashboard uses sum(cost_delta_usd) across all users for accurate totals
